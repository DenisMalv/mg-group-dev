...
import StepSixFormError from "./StepSixFormError";
...
const CalculatorForm = ({ type, typeIsShow, setTypeIsShow, isShow, setIsShow }) => {
	...
	const onChangeInput = (name, value) => {
		...
		type === "build" ? setData({ ...data, [name]: value }) : setDrillData({ ...drillData, [name]: value });
	};
	...
	const onSubmit = async (names, data) => {
		...
		if (!nameRegex.test(data[names[0]]) || !phoneRegex.test(data[names[1]?.trim()]) || !captchaToken) {
			names.forEach((el) => {
				if (el === "name") {
			...
			setError((prevState) => ({ ...prevState, captcha: true }));
			return;
		}
		if (!captchaToken) {
			return;
	...
	const totalDrillSumm = () => {
		...
		return Math.round(
			diametrPrice * count * underTwoMetres * hardDrill * highArm * m_400 * m_500 * noWater * winter * holiday
		);
	};
	...
	return type === "build" ? (
		<form
			action="#"
			className="calc-form bg-card p-lr-12-to-32 gap-44-to-68 br-16-to-24"
			onSubmit={(e) => {
				e.preventDefault();
			}}
		>
			...
			{step === 1 && (
				<StepOneFormContent
					...
					setOpen={setOpen}
					data={data}
					updData={onChangeInput}
					isShow={isShow}
					setIsShow={setIsShow}
				/>
			)}
			{step === 2 && (
				<StepTwoFormContent stage={"stage"} data={data} updData={onChangeInput} isShow={isShow} setIsShow={setIsShow} />
			)}
			{step === 3 && (
				<StepThreeFormContent
					area={"area"}
					data={data}
					updData={onChangeInput}
					isShow={isShow}
			...
			{step === 4 && (
				<div className="flex gap-12-to-32">
					<button
						className={`btn-primary calc-next-step  ${typeIsShow ? "show" : ""}`}
						type="button"
						onClick={() => onSubmit(["name", "phone"], data)}
					>
		...
		<form action="#" className="calc-form bg-card p-lr-12-to-32 gap-44-to-68 br-16-to-24">
			...
			{step !== 1 && step !== 3 && step !== 6 && (
				<button
					className={`btn-primary calc-next-step  ${typeIsShow ? "show" : ""}`}
					type="button"
					onClick={() => onSubmit(["name", "phone"], drillData)}
				>
					Зв’язатися з нами
				</button>
			)}
